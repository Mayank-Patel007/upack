...
const EditCategory = () => {
  ...
  const [navaltText, setNavAltText] = useState("");
  const [currentPhoto, setCurrentPhoto] = useState("");
  const [currentNavPhoto, setCurrentNavPhoto] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const { category, photo, navphoto, alt, navalt, slug, metatitle, metadescription, metakeywords, metalanguage, metacanonical, metaschema, otherMeta, changeFreq, priority } = response.data;

        setCategory(category);
        setCurrentPhoto(photo);
        setCurrentNavPhoto(navphoto);
        setAltText(alt);
        setNavAltText(navalt);
        setSlug(slug);
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("navalt", navaltText);
    ...
    if (navphoto) {
      formData.append("navphoto", navphoto);
    } else {
      formData.append("navphoto", currentNavPhoto);
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="p-4">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-400 px-3 pb-3">Note: The image should be 320x320 px, or it will be resized automatically, which may alter its appearance.</p>
        <input
          type="file"
          name="photo"
          id="photo"
      ...
      </div>
      {
        ...
        && <div className="mb-8">
          <label htmlFor="photo" className="block font-semibold mb-2">Photo for Navbar</label>
          <input
            type="file"
            name="photo"
            id="photo"
            onChange={handleNavPhotoChange}
            className="border rounded focus:outline-none"
            accept="image/*"
          />
          {photoError && <p className="text-red-500 text-sm mt-2">{photoError}</p>}
          {(navphoto || currentNavPhoto) && (
            <div className="mt-2 w-56 relative group">
              <img
                src={navphoto ? URL.createObjectURL(navphoto) : `http://localhost:3006/api/logo/download/${currentNavPhoto}`}
                alt={navaltText}
                className="h-32 w-56 object-cover"
              />
              <button
                type="button"
                onClick={handleDeleteNavImage}
                className="absolute top-4 right-2 bg-red-500 text-white rounded-md p-1 size-6 flex items-center justify-center hover:bg-red-600 focus:outline-none"
              >
                X
              </button>
              <div className="mb-4">
                ...
                <input
                  ...
                  id="alt"
                  value={navaltText}
                  onChange={(e) => setNavAltText(e.target.value)}
                  className="w-full p-2 border rounded focus:outline-none"
                  required
                />
              </div>
