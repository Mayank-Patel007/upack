...
const insertBanner = async (req, res) => {
  try {
    const { section, title, details, details2, status, priority } = req.body;
    const photos = req.files['photo'] ? req.files['photo'].map(file => file.filename) : [];
    const altTexts = req.body.alt || [];
...
const updateBanner = async (req, res) => {
  const { id } = req.query;
  const { section, title, details, details2, status, priority, alt } = req.body;
  ...
  try {
    ...
    banner.details2 = details2; // Update details2
...
const getHomeActiveBanners = async (req, res) => {
  try {
    const banners = await Banner.find({ section: 'Home', status: 'active' });
    res.status(200).json({
      data: banners,
      total: banners.length,
      message: "Home active banners fetched successfully",
    });
  } catch (err) {
    console.error("Error fetching home active banners:", err);
    res.status(400).send(err);
  }
};
...
const getBannersBySection = async (req, res) => {
  try {
      ...
      // Fetch banners where section matches the provided category name
      ...
      console.error('Error fetching banners by section:', err);
      res.status(500).json({ message: 'Server error' });
  }
};
