...
const insertCategory = async (req, res) => {
  ...
  // If req.file is undefined, set photo as an empty string
  const photo = req.files && req.files['photo'] ? req.files['photo'][0].filename : "";
...
const insertSubCategory = async (req, res) => {
  ...
  const { category, alt,navalt, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq } = req.body;
  const photo = req.files && req.files['photo'] ? req.files['photo'][0].filename : "";
  const navphoto = req.files && req.files['navphoto'] ? req.files['navphoto'][0].filename : "";
  ...
  try {
    ...
    categoryDoc.subCategories.push({ category, alt,navalt, photo,navphoto, slug, metatitle, metadescription, metakeywords, metacanonical, metalanguage, metaschema, otherMeta, url, priority, changeFreq });
...
const updateCategory = async (req, res) => {
  ...
  if (req.files && req.files['photo']) {
    photo = req.files['photo'][0].filename; // Get the uploaded photo file
  }
...
const updateSubCategory = async (req, res) => {
  ...
  let photo = req.body.photo;
  let navphoto = req.body.navphoto;
  ...
  // if (req.files && req.files['navphoto']) {
  //   navphoto = req.files['navphoto'][0].filename; // Get the uploaded navphoto file
  ...
  try {
    ...
    if (photo !== undefined) subCategory.photo = photo;
    if (navphoto !== undefined) subCategory.navphoto = navphoto;
    if (alt !== undefined) subCategory.alt = alt;
    if (navalt !== undefined) subCategory.navalt = navalt;
    ...
    if (metakeywords !== undefined) subCategory.metakeywords = metakeywords;
    if (metacanonical !== undefined) subCategory.metacanonical = metacanonical;
    if (metalanguage !== undefined) subCategory.metalanguage = metalanguage;
    if (metaschema !== undefined) subCategory.metaschema = metaschema;
    if (otherMeta !== undefined) subCategory.otherMeta = otherMeta;
    if (url !== undefined) subCategory.url = url;
    if (priority !== undefined) subCategory.priority = priority;
    if (changeFreq !== undefined) subCategory.changeFreq = changeFreq;
...
const updatesubsubcategory = async (req, res) => {
  ...
  try {
    ...
    if (category !== undefined) subSubCategory.category = category;
    if (photo !== undefined) subSubCategory.photo = photo;
    if (alt !== undefined) subSubCategory.alt = alt;
    if (slug !== undefined) subSubCategory.slug = slug;
    if (metatitle !== undefined) subSubCategory.metatitle = metatitle;
    if (metadescription !== undefined) subSubCategory.metadescription = metadescription;
    if (metakeywords !== undefined) subSubCategory.metakeywords = metakeywords;
    if (metacanonical !== undefined) subSubCategory.metacanonical = metacanonical;
    if (metalanguage !== undefined) subSubCategory.metalanguage = metalanguage;
    if (metaschema !== undefined) subSubCategory.metaschema = metaschema;
    if (otherMeta !== undefined) subSubCategory.otherMeta = otherMeta;
    if (url !== undefined) subSubCategory.url = url;
    if (priority !== undefined) subSubCategory.priority = priority;
    if (changeFreq !== undefined) subSubCategory.changeFreq = changeFreq;
    await categoryDoc.save();
    res.status(200).json(categoryDoc);
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
...
const deletesubcategory = async (req, res) => {
  ...
  try {
    ...
    const NavphotoPath = path.join(__dirname, '../logos', subCategory.navphoto);
    deleteFile(photoPath);
    deleteFile(NavphotoPath);
...
const getCategoriesOnly = async (req, res) => {
  try {
    // Find all categories and only return the 'category' field
    const categories = await ProductCategory.find().select('category');
...
module.exports = {getAllSubCategoryDetails, insertCategory, insertSubCategory, insertSubSubCategory, updateCategory, updateSubCategory, updatesubsubcategory, deletecategory, deletesubcategory, deletesubsubcategory, getAll, getSpecificCategory, getSpecificSubcategory, getSpecificSubSubcategory, fetchCategoryUrlPriorityFreq, editCategoryUrlPriorityFreq, fetchCategoryUrlPriorityFreqById, fetchCategoryUrlmeta, editCategoryUrlmeta, fetchCategoryUrlmetaById, getCategoriesOnly };